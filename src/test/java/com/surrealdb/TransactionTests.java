...
/**
 * Tests for client-side transaction support: {@link Surreal#beginTransaction()},
 * {@link Transaction#query(String)}, {@link Transaction#commit()}, {@link Transaction#cancel()}.
 */
public class TransactionTests {
    ...
    void transaction_isolation_recordNotVisibleUntilCommit() {
        try (Surreal surreal = new Surreal()) {
            ...
            // From main instance, record created in transaction should not be visible yet (isolation)
            ...
            assertTrue(
                first.isNone() || (first.isArray() && first.getArray().len() == 0),
            ...
            assertTrue(afterFirst.isArray() && afterFirst.getArray().len() >= 1, "Record should be visible after commit");
        }
    }
    ...
    void transaction_cancel_rollsBackData() {
        try (Surreal surreal = new Surreal()) {
            ...
            assertNotNull(response);
            // After cancel, created data should not be visible (rollback). Some backends may behave differently.
