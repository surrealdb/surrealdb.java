...
pub extern "system" fn Java_com_surrealdb_ValueMut_newDecimal<'local>(
    ...
    with_env_body!(env, env, {
        ...
        let d = match Decimal::from_str(&s) {
            Ok(d) => d,
            Err(e) => return SurrealError::SurrealDBJni(e.to_string()).exception(env, || 0),
        };
...
pub extern "system" fn Java_com_surrealdb_ValueMut_newUuid<'local>(
    ...
    with_env_body!(env, env, {
        ...
        SurrealError::NullPointerException("Uuid").exception(env, || 0)
    })
}
...
pub extern "system" fn Java_com_surrealdb_ValueMut_newObjectOf<'local>(
    ...
    with_env_body!(env, env, {
        ...
        for ptr in ptrs {
            let (key, value) = take_entry_mut_instance!(env, ptr, || 0);
...
pub extern "system" fn Java_com_surrealdb_ValueMut_newFile<'local>(
    ...
    with_env_body!(env, env, {
        let bucket = get_rust_string!(env, bucket, || 0);
...
pub extern "system" fn Java_com_surrealdb_ValueMut_toString<'local>(
    ...
    with_env_body!(env, env, {
        let value = get_value_mut_instance!(env, ptr, null_mut);
...
pub extern "system" fn Java_com_surrealdb_ValueMut_hashCode<'local>(
    ...
    with_env_body!(env, env, {
        let value = get_value_mut_instance!(env, ptr, || 0);
...
pub extern "system" fn Java_com_surrealdb_ValueMut_equals<'local>(
    ...
    with_env_body!(env, env, {
        let v1 = get_value_mut_instance!(env, ptr1, || false as jboolean);
        let v2 = get_value_mut_instance!(env, ptr2, || false as jboolean);
