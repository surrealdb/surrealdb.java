...
/**
 * Client-side transaction. Use {@link Surreal#beginTransaction()} to start a transaction.
 * Operations (e.g. {@link #query(String)}) run within the transaction until {@link #commit()}
...
public class Transaction extends Native {
    ...
    /**
     * Commits the transaction. After this call, the transaction is completed and must not be used.
     */
    public void commit() {
        commit(getPtr());
        moved();
    ...
    /**
     * Cancels (rolls back) the transaction. After this call, the transaction is completed and must not be used.
     */
    public void cancel() {
        cancel(getPtr());
        moved();
